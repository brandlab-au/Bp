...
/*
    ...
    *
    */
function wppb_woosync_register_settings() {
    register_setting( 'wppb_woosync_settings', 'wppb_woosync_settings', 'wppb_woosync_settings_sanitize' );
}
if ( is_admin() ) {
    add_action('admin_init', 'wppb_woosync_register_settings');
}
 ...
 * @return void
 */
function wppb_woosync_settings_submenu_page() {
    add_submenu_page( 'profile-builder', __( 'WooCommerce Sync', 'profile-builder-woocommerce-add-on' ), __( 'WooCommerce Sync', 'profile-builder-woocommerce-add-on' ), 'manage_options', 'profile-builder-woocommerce-sync', 'wppb_woosync_settings_content' );
}
add_action( 'admin_menu', 'wppb_woosync_settings_submenu_page', 20 );  // this priority adds WooCommerce Sync below the Addons tab in PB menu

// set default values for WooCommerce Sync settings page
function wppb_woosync_settings_defaults(){
 ...
 * @return string
 */
function wppb_woosync_settings_content() {
    wppb_woosync_settings_defaults();
    ?>
    <div class="wrap wppb-wrap">
        <form method="post" action="options.php">
            ...
            <h2><?php _e( 'WooCommerce Sync', 'profile-builder-woocommerce-add-on' ); ?></h2>

            <table class="form-table">

                <tr>
                    <th scope="row">
                        <?php _e( 'Choose Register form to display on My Account page:', 'profile-builder-woocommerce-add-on' ); ?>
                    </th>
                    <td>
                        <select name="wppb_woosync_settings[RegisterForm]" class="wppb-select">
                            <option value=""> <?php _e( 'None', 'profile-builder-woocommerce-add-on' ); ?></option>
                            <option value="wppb-default-register" <?php if ( $wppb_woosync_settings['RegisterForm'] == 'wppb-default-register' ) echo 'selected'; ?>><?php _e( 'Default Register', 'profile-builder-woocommerce-add-on' ); ?></option>
                            <?php
                            $args = array(
                        ...
                        <p class="description">
                            <?php _e( 'Select which Profile Builder Register form to display on My Account page from WooCommerce. <br/> This will also add the Profile Builder Login form to MyAccount page.', 'profile-builder-woocommerce-add-on' ); ?>
                        </p>
                    </td>
                </tr>
                ...
                <tr>
                    <th scope="row">
                        <?php _e( 'Choose Edit Profile form to display on My Account page:', 'profile-builder-woocommerce-add-on' ); ?>
                    </th>
                    <td>
                        <select name="wppb_woosync_settings[EditProfileForm]" class="wppb-select">
                            <option value=""> <?php _e( 'None', 'profile-builder-woocommerce-add-on' ); ?></option>
                            <option value="wppb-default-edit-profile" <?php if ( $wppb_woosync_settings['EditProfileForm'] == 'wppb-default-edit-profile' ) echo 'selected'; ?>><?php _e( 'Default Edit Profile', 'profile-builder-woocommerce-add-on' ); ?></option>
                            <?php
                            $args = array(
                                'post_type' => 'wppb-epf-cpt',
                                'post_status' => 'publish',
                        ...
                        <p class="description">
                            <?php _e( 'Select which Profile Builder Edit-profile form to display on My Account page from WooCommerce.', 'profile-builder-woocommerce-add-on' ); ?>
                        </p>
                    </td>
                </tr>
 ...
 * @since v.1.2.0
 */
function wppb_woosync_settings_sanitize( $wppb_woosync_settings ) {
 ...
 * @since v.1.2.0
 */
function wppb_woosync_settings_admin_notices() {
    settings_errors( 'wppb_woosync_settings' );
}
add_action( 'admin_notices', 'wppb_woosync_settings_admin_notices' );
