...
class backup_database_Backup {
	...
	public function __construct(){
		...
		if( $this->errors == false){
			print '<script>setTimeout(function(){ window.location.href = "'.admin_url() .'admin.php?page=backup_database_manage"}, 3000);</script> <p>The backup has been started. The window should refresh in a few moments. If not please click <a href="'.admin_url() .'admin.php?page=backup_database_manage"> Here </a>. </p>';
			flush(); sleep(1);
		}
...
function check_backup_limit()
{
	...
	$tables =array_merge($wp_backup_default_tables,$other_tables);
	else
