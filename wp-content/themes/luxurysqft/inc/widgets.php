...
class Custom_Widget_Text extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array(
				...
				'description' => __( 'Arbitrary text or HTML', 'luxurysqft' ),
				);
		$control_ops = array(
				'width'  => 400,
				'height' => 350,
				);
		parent::__construct( 'text', __( 'Text', 'luxurysqft' ), $widget_ops, $control_ops );
	}
...
?>
		<p><label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'luxurysqft' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" /></p>

		<textarea class="widefat" rows="16" cols="20" id="<?php echo $this->get_field_id( 'text' ); ?>" name="<?php echo $this->get_field_name( 'text' ); ?>"><?php echo $text; ?></textarea>

		<p><input id="<?php echo $this->get_field_id( 'filter' ); ?>" name="<?php echo $this->get_field_name( 'filter' ); ?>" type="checkbox" <?php checked( isset( $instance['filter'] ) ? $instance['filter'] : 0 ); ?> />&nbsp;<label for="<?php echo $this->get_field_id( 'filter' ); ?>"><?php _e( 'Automatically add paragraphs', 'luxurysqft' ); ?></label></p>
<?php
	}
}
...
class Widget_Recent_Posts_From_Category extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array(
				...
				'description' => __( 'The most recent posts from specific category on your site', 'luxurysqft' ),
				);
		parent::__construct( 'recent-posts-from-category', __( 'Recent Posts From Specific Category', 'luxurysqft' ), $widget_ops );
	...
	function widget( $args, $instance ) {
		...
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Recent Posts', 'luxurysqft' ) : $instance['title'], $instance, $this->id_base );
...
<p>
<label>
<?php _e( 'Category', 'luxurysqft' ); ?>:
<?php wp_dropdown_categories( array( 'name' => $this->get_field_name("cat"), 'selected' => $cat ) ); ?>
</label>
</p>

<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of posts to show:', 'luxurysqft' ); ?></label>
