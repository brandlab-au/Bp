...
function seo_warning() {
	...
	$message = __( 'You are blocking access to robots. You must go to your <a href="%s">Reading</a> settings and uncheck the box for Search Engine Visibility.', 'luxurysqft' );
...
function theme_localization () {
	load_theme_textdomain( 'luxurysqft', get_template_directory() . '/languages' );
}
...
function wp_admin_bar_valid_search_menu( $wp_admin_bar ) {
	...
	$form .= '<input type="submit" class="adminbar-button" value="' . __( 'Search', 'luxurysqft' ) . '"/>';
	$form .= '</div></form>';
...
function theme_get_the_password_form() {
	...
	<p>' . __( 'This content is password protected. To view it please enter your password below:', 'luxurysqft' ) . '</p>
	<p><label for="' . $label . '">' . __( 'Password:', 'luxurysqft' ) . '</label> <input name="post_password" id="' . $label . '" type="password" size="20" /> <input type="submit" name="Submit" value="' . esc_attr__( 'Submit', 'luxurysqft' ) . '" /></p></form>
	';
	return $output;
}
