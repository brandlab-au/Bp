...
<div id="wrapper" class="wrapper">
	...
	<header id="header">
		<nav class="navbar navbar-default">
					...
					<div class="right_wrapper clearfix">
						...
						<ul class="right-menu">
							...
							<?php if (is_user_logged_in()) : ?>
								<li>
									<a href="<?php echo get_permalink(get_option('woocommerce_myaccount_page_id')); ?>/settings/"><?php _e('ACCOUNT', 'luxurysqft'); ?></a>
									/
								</li>
								<li>
			...
			if (strpos(strtolower($_SERVER['REQUEST_URI']), '/buy') !== false ||
					strpos(strtolower($_SERVER['REQUEST_URI']), '/rent') !== false ||
					strpos(strtolower($_SERVER['REQUEST_URI']), '/location') !== false ||
					strpos(strtolower($_SERVER['REQUEST_URI']), '/area') !== false
			) {
				$class = '';
			}
			...
			<div class="wrapper_bottom <?php print isset($_COOKIE['StorageCookie']) && $_COOKIE['StorageCookie'] != '[]' ? '' : $class; ?>">
				<div class="left">
					<div class="left_wrapper">
						<?php if (is_singular('apartments')) : ?>
							<a class="back-link-top"
							   href="<?php echo get_apartments_url() ?>" ><?php _e('back', 'luxurysqft'); ?></a>
						<?php endif; ?>

						<span class="saved-items <?php print isset($_COOKIE['StorageCookie']) && $_COOKIE['StorageCookie'] != '[]' ? '' : $class; ?>"><a
				...
				<div class="right <?php print $class; ?>">
					<div class="right_wrapper navigation-list">
						...
						<ul class="form-container form-container-mobile">
							...
							<li>
								<select name="beds" class="filter-drop">
									...
									<option value="1" <?php if (isset($_GET['beds'])) selected($_GET['beds'], 1); ?>>
										01
									</option>
									<option value="2" <?php if (isset($_GET['beds'])) selected($_GET['beds'], 2); ?>>
										02
									</option>
									<option value="3" <?php if (isset($_GET['beds'])) selected($_GET['beds'], 3); ?>>
										03
									</option>
									<option value="4" <?php if (isset($_GET['beds'])) selected($_GET['beds'], 4); ?>>
										04
									</option>
									<option value="5" <?php if (isset($_GET['beds'])) selected($_GET['beds'], 5); ?>>
										05
									</option>
									<option value="6" <?php if (isset($_GET['beds'])) selected($_GET['beds'], 6); ?>>
										06
									</option>
									<option value="7" <?php if (isset($_GET['beds'])) selected($_GET['beds'], 7); ?>>
										07
									</option>
									<option value="8" <?php if (isset($_GET['beds'])) selected($_GET['beds'], 8); ?>>
										08
									</option>
									<option value="9" <?php if (isset($_GET['beds'])) selected($_GET['beds'], 9); ?>>
										09
									</option>
									<option value="10" <?php if (isset($_GET['beds'])) selected($_GET['beds'], 10); ?>>
										10+
									</option>
								</select>
							</li>
							<li>
								<select name="price" class="filter-drop">
									...
									<option class="price-rent"
									        value="0-1000" <?php if (isset($_GET['price'])) selected($_GET['price'], '0-1000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										0 - <?php _e('AED', 'luxurysqft'); ?> 1,000
									</option>
									<option class="price-rent"
									        value="1000-2000" <?php if (isset($_GET['price'])) selected($_GET['price'], '1000-2000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										1,000 - <?php _e('AED', 'luxurysqft'); ?> 2,000
									</option>
									<option class="price-rent"
									        value="2000-3000" <?php if (isset($_GET['price'])) selected($_GET['price'], '2000-3000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										2,000 - <?php _e('AED', 'luxurysqft'); ?> 3,000
									</option>
									<option class="price-rent"
									        value="3000-4000" <?php if (isset($_GET['price'])) selected($_GET['price'], '3000-4000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										3,000 - <?php _e('AED', 'luxurysqft'); ?> 4,000
									</option>
									<option class="price-rent"
									        value="4000-5000" <?php if (isset($_GET['price'])) selected($_GET['price'], '4000-5000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										4,000 - <?php _e('AED', 'luxurysqft'); ?> 5,000
									</option>
									<option class="price-rent"
									        value="5000-6000" <?php if (isset($_GET['price'])) selected($_GET['price'], '5000-6000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										5,000 - <?php _e('AED', 'luxurysqft'); ?> 6,000
									</option>
									<option class="price-rent"
									        value="6000-7000" <?php if (isset($_GET['price'])) selected($_GET['price'], '6000-7000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										6,000 - <?php _e('AED', 'luxurysqft'); ?> 7,000
									</option>
									<option class="price-rent"
									        value="7000-8000" <?php if (isset($_GET['price'])) selected($_GET['price'], '7000-8000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										7,000 - <?php _e('AED', 'luxurysqft'); ?> 8,000
									</option>
									<option class="price-rent"
									        value="8000-9000" <?php if (isset($_GET['price'])) selected($_GET['price'], '8000-9000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										8,000 - <?php _e('AED', 'luxurysqft'); ?> 9,000
									</option>
									<option class="price-rent"
									        value="9000-10000" <?php if (isset($_GET['price'])) selected($_GET['price'], '9000-10000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										9,000 - <?php _e('AED', 'luxurysqft'); ?> 10,000
									</option>
									<option class="price-rent"
									        value="10000-9999999999" <?php if (isset($_GET['price'])) selected($_GET['price'], '10000-9999999999'); ?>><?php _e('AED', 'luxurysqft'); ?>
										10,000+
									</option>

									<option class="price-buy"
									        value="0-3000" <?php if (isset($_GET['price'])) selected($_GET['price'], '0-3000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										0 - <?php _e('AED', 'luxurysqft'); ?> 3,000
									</option>
									<option class="price-buy"
									        value="3000-5000" <?php if (isset($_GET['price'])) selected($_GET['price'], '3000-5000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										3,000 - <?php _e('AED', 'luxurysqft'); ?> 5,000
									</option>
									<option class="price-buy"
									        value="5000-8000" <?php if (isset($_GET['price'])) selected($_GET['price'], '5000-8000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										5,000 - <?php _e('AED', 'luxurysqft'); ?> 8,000
									</option>
									<option class="price-buy"
									        value="8000-10000" <?php if (isset($_GET['price'])) selected($_GET['price'], '8000-10000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										8,000 - <?php _e('AED', 'luxurysqft'); ?> 10,000
									</option>
									<option class="price-buy"
									        value="10000-15000" <?php if (isset($_GET['price'])) selected($_GET['price'], '10000-15000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										10,000 - <?php _e('AED', 'luxurysqft'); ?> 15,000
									</option>
									<option class="price-buy"
									        value="15000-20000" <?php if (isset($_GET['price'])) selected($_GET['price'], '15000-20000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										15,000 - <?php _e('AED', 'luxurysqft'); ?> 20,000
									</option>
									<option class="price-buy"
									        value="20000-50000" <?php if (isset($_GET['price'])) selected($_GET['price'], '20000-50000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										20,000 - <?php _e('AED', 'luxurysqft'); ?> 50,000
									</option>
									<option class="price-buy"
									        value="50000-100000" <?php if (isset($_GET['price'])) selected($_GET['price'], '50000-100000'); ?>><?php _e('AED', 'luxurysqft'); ?>
										50,000 - <?php _e('AED', 'luxurysqft'); ?> 100,000
									</option>
									<option class="price-buy"
									        value="100000-9999999999" <?php if (isset($_GET['price'])) selected($_GET['price'], '10000-9999999999'); ?>><?php _e('AED', 'luxurysqft'); ?>
										100,000+
									</option>
								</select>
							</li>
						...
						<form action="<?php echo $apartments_url ?>" class="" method="get">
											...
											<div class="nano">
												<div class="nano-content">
													<ul class="filter-drop">
														<?php foreach ($terms as $term) :
															if ($term->parent == 0) continue ?>
															<li value="<?php echo $term->term_id ?>" <?php if (isset($_GET['city'])) selected($_GET['city'], $term->term_id); ?>>
							...
							?>
							<input type="hidden" id="buy" name="buy" value="<?php print isset($type) ? $type : ''; ?>"/>
						...
						<?php else : ?>
							...
							<ul class="property-list single-apartments">
								...
								<li><a onclick="window.print(); return false;" href="#"><?php _e('Print', 'luxurysqft'); ?></a>
								</li>
								<li><a data-product-id="<?php the_ID() ?>" href="#"><?php _e('Save', 'luxurysqft'); ?></a></li>
	...
	<?php
	if ((is_page(36) or (is_page(14310)) or is_single()) && ! is_singular('apartments') && ! is_product()): ?>
		<?php if (is_single()):
			$id = get_the_ID();
		elseif (is_page(36)):
			$id = 36;
		...
		<div class="banner-home">
			<picture>
				<?php $image_info = get_x2_image($id, array('thumbnail_1900x722', 'thumbnail_3800x1444')); ?>
				<!--[if IE 9]>
				<video style="display: none;"><![endif]-->
				<source srcset="<?php echo $image_info['thumbnail_1900x722'] ?>, <?php echo $image_info['thumbnail_3800x1444'] ?> 2x">
				<!--[if IE 9]></video><![endif]-->
				<img src="<?php echo $image_info['thumbnail_1900x722'] ?>" alt="<?php echo $image_info['alt'] ?>">
			</picture>
		</div>
	<?php endif; ?>
